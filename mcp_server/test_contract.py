...
def main() -> int:
    ...
    composed = "".join(
        ...
        for name in ("sheet.py", "arch.py", "furniture.py", "annotation.py", "civil.py", "profile.py")
    )
    ...
    silent = sorted(t for t in exposed if not re.search(
        r"def\s+" + t + r"\b[\s\S]{0,6000}?(acad\.call|sheet_mod\.|arch_mod\.|fur_mod\.|ann_mod\.|civil_mod\.|profile_mod\.)",
        server_src))
    if silent:
